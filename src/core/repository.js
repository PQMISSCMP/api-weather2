...
const getWeather = async(latitud,longitud , secretkey) => {
    let jsonApiWeather;
    try {
        const API_WEATHER = `${process.env.URL_API_WEATHER}/${secretkey}/${latitud},${longitud}`;       
        ...
        jsonApiWeather = JSON.parse(resultApiWeather);
        return { temperatura: jsonApiWeather.currently.temperature, fechahora: jsonApiWeather.currently.time };
    } catch (issue) {
        throw (issue);
    }
}
...
const getInfoCountry = async(shortNameCountry) =>{
    const queryApi = `${process.env.URL_API_COUNTRIES}/${shortNameCountry}`;
    let jsonCountry;
    try {
